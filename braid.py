  ...
  def update_drawing(self):
    ...
    for pi,p in enumerate(paths):
      ...
      for e in this_path_picture:
        if e[0] == 'line':
          canvas_pts = [(canvas_center[1]-draw_scale*x if i%2==1 else draw_scale*x) for i,x in enumerate(e[1:])]
          self.canvas.create_line(*canvas_pts, width=path_thickness, fill=this_path_col)
        else:
          n, c, r, a1, a2 = e
          a1 *= 180.0/math.pi
          a2 *= 180.0/math.pi
